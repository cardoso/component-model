...
handle ::= id
         ...
         | 'call-scoped' id
         | 'call-scoped' 'rc' '<' id '>'
         | 'call-scoped' 'borrow' '<' id '>'
         | 'call-scoped' 'use' '<' id '>'
         | 'child' id ( 'of' id )?
         | 'child' 'rc' '<' id '>' ( 'of' id )?
         | 'child' 'borrow' '<' id '>' ( 'of' id )?
         | 'child' 'use' '<' id '>' ( 'of' id )?
```

The `id` case translates to the component handle type `(unique id)`, where `id`
must resolve to a resource type. Thus, `unique` is the "default" handle type.
The other `H<id>` cases (where `H` is one of `rc`, `borrow` or `use`) map
to the analogous `handle` type abbreviations. The full 2x2 matrix of ownership
and exclusivity is:
...
The `call-scoped` prefix adds the `call` scope to the succeeding handle type.
...
`call-scoped use`. For example, the compound definition:
```
resource file {
...
```
resource file
%[method]file.read-sync: func(self: call-scoped use<file>, n: u32) -> list<u8>
%[method]file.read-async: func(self: call-scoped use<file>, n: u32) -> child input-stream of self
```
...
TODO: introduce `destructor` as resource sugar for `self: call-scoped R`
